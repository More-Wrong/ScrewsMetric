...
module M_BoltOnSurface(boltType, size, length, ERR=0, silent = false){
   if(boltType == allenBolt){
      M_AllenBolt(size, length, ERR);
      if(!silent)echo("Allen bolt not being inset, may look slightly dodgy");
   }else if(boltType == allenButtonBolt){
      M_AllenButtonBolt(size, length, ERR);
   }else if(boltType == allenCountersunkBolt){
      M_AllenCountersunkBolt(size, length, ERR);
   }else if(boltType == hexHeadBolt){
      M_HexHeadBolt(size, length, ERR);
   }else if(boltType == hexFlangeBolt){
      M_HexHeadFlangeBolt(size, length, ERR);
   }else if(boltType == hexHeadBoltAllowSpin){
      M_HexHeadBoltHoleAllowingSpin(size, length, M(size, hexHeadBoltHeadH), ERR);
   }else{
...
module M_BoltFlushWithSurface(boltType, size, length, ERR=0, override = false, silent = false){
   if(boltType == allenBolt){
      translate([0, 0, -M(size, allenBoltHeadH)])AllenBolt(size, length-M(size, allenBoltHeadH), ERR);
   }else if(boltType == allenButtonBolt){
      if(!override){
         ...
         translate([0, 0, -M(size, allenButtonBoltHeadH)])M_AllenButtonBolt(size, length-M(size, allenButtonBoltHeadH), ERR);
      }
   }else if(boltType == allenCountersunkBolt){
      M_AllenCountersunkBolt(size, length, ERR);
   }else if(boltType == hexHeadBolt){
      if(!silent)echo("Hex bolt being inset, this will not allow it to be turned, it will be captive");
      translate([0, 0, -M(size, hexHeadBoltHeadH)])M_HexHeadBolt(size, length-M(size, hexHeadBoltHeadH), ERR);
   }else if(boltType == hexFlangeBolt){
      if(!override){
         ...
         translate([0, 0, -M(size, hexFlangeHeadH)])M_HexHeadFlangeBoltHole(size, length-M(size, hexFlangeHeadH), M(size, hexFlangeHeadH), ERR);
      }
   }else if(boltType == hexHeadBoltAllowSpin){
      if(!silent)echo("WARNING: Hex bolt being inset, this can be turned, but there is no room to access it");
      translate([0, 0, -M(size, hexHeadBoltHeadH)])M_HexHeadBoltHoleAllowingSpin(size, length-M(size, hexHeadBoltHeadH), M(size, hexHeadBoltHeadH), ERR);
   }else{
      echo("ERROR: Bolt type not recognised, proceding with default bolt");
      translate([0, 0, -M(size, allenBoltHeadH)])M_AllenBolt(size, length-M(size, allenBoltHeadH), ERR);
   }
}

module M_BoltNormalWithSurface(boltType, size, length, ERR=0, silent = false){
   if(boltType == allenBolt){
      translate([0, 0, -M(size, allenBoltHeadH)])M_AllenBolt(size, length-M(size, allenBoltHeadH), ERR);
   }else if(boltType == allenButtonBolt){
      M_AllenButtonBolt(size, length, ERR);
...
module M_BoltInHoleFromTop(boltType, size, length, holeDepth, ERR=0, silent = false){
   if(boltType == allenBolt){
      translate([0, 0, -M(size, allenBoltHeadH)])M_AllenBoltHole(size, length-M(size, allenBoltHeadH), holeDepth+M(size, allenBoltHeadH), ERR);
   }else if(boltType == allenButtonBolt){
      if(!silent)echo("WARNING: Button bolts do not inset well, using another form of bolt is recommended");
      translate([0, 0, -M(size, allenButtonBoltHeadH)])M_AllenButtonBoltHole(size, length-M(size, allenButtonBoltHeadH), holeDepth+M(size, allenButtonBoltHeadH), ERR);
   }else if(boltType == allenCountersunkBolt){
      M_AllenCountersunkBoltHole(size, length, holeDepth, ERR);
   }else if(boltType == hexHeadBolt){
      if(!silent)echo("Hex bolt being inset, this will not allow it to be turned, it will be captive");
      translate([0, 0, -M(size, hexHeadBoltHeadH)])M_HexHeadBoltHole(size, length-M(size, hexHeadBoltHeadH), holeDepth+M(size, hexHeadBoltHeadH), ERR);
   }else if(boltType == hexFlangeBolt){
      ...
      translate([0, 0, -M(size, hexFlangeHeadH)])M_HexHeadFlangeBoltHole(size, length-M(size, hexFlangeHeadH), M(size, hexFlangeHeadH)+holeDepth, ERR);
   }else if(boltType == hexHeadBoltAllowSpin){
      if(!silent)echo("WARNING: Hex bolt being inset, this can be turned, but there is no room to access it");
      translate([0, 0, -M(size, hexHeadBoltHeadH)])M_HexHeadBoltHoleAllowingSpin(size, length-M(size, hexHeadBoltHeadH), holeDepth+M(size, hexHeadBoltHeadH), ERR);
   }else{
      echo("ERROR: Bolt type not recognised, proceding with default bolt");
      translate([0, 0, -M(size, allenBoltHeadH)])M_AllenBoltHole(size, length-M(size, allenBoltHeadH), holeDepth+M(size, allenBoltHeadH), ERR);
   }
}
module M_BoltInHoleFromBottom(boltType, size, length, holeDepth, ERR=0, silent = false){
   if(boltType == allenBolt){
      ...
      translate([0, 0, M(size, allenCountersunkBoltHeadH)])M_AllenCountersunkBoltHole(size, length+M(size, allenCountersunkBoltHeadH), holeDepth-M(size, allenCountersunkBoltHeadH), ERR);
   }else if(boltType == hexHeadBolt){
      if(!silent)echo("Hex bolt being inset, this will not allow it to be turned, it will be captive");
...
module M_BoltInHoleFromNormal(boltType, size, length, holeDepth, ERR=0, silent = false){
   if(boltType == allenBolt){
      translate([0, 0, -M(size, allenBoltHeadH)])M_AllenBoltHole(size, length-M(size, allenBoltHeadH), holeDepth+M(size, allenBoltHeadH), ERR);
   }else if(boltType == allenButtonBolt){
      if(!silent)echo("WARNING: Button bolts do not inset well, using another form of bolt is recommended");
